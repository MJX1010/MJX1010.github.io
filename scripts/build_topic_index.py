#!/usr/bin/env python3
"""
Build a unified topic index for a directory of Markdown notes.

The script scans topic notes, extracts core bullet points from key sections,
and writes a navigable summary note.
"""
...
DEFAULT_TITLE = "Unity 知识索引"
DEFAULT_TOPIC_TAG = "Unity"
DEFAULT_SCRIPT_NAME = "scripts/build_topic_index.py"
...
def parse_list_arg(raw: str) -> set[str]:
    if not raw.strip():
        return set()
    return {part.strip() for part in raw.split(",") if part.strip()}


def find_notes(content_dir: Path, include_glob: str, skip_names: set[str]) -> list[Path]:
    notes = []
    for path in sorted(content_dir.glob(include_glob)):
...
def build_index(
    ...
    *,
    ...
    lines = [
        ...
        "---",
        "",
        f"> 本文件由 `{script_name}` 自动生成，生成时间：{now_date()}。",
        ">",
        f"> 用途：{summary}",
        "",
        "## 笔记总览",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a topic knowledge index.")
    parser.add_argument("--content-dir", type=Path, default=DEFAULT_CONTENT_DIR)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--max-points", type=int, default=DEFAULT_MAX_POINTS)
    parser.add_argument("--title", default=DEFAULT_TITLE)
    parser.add_argument("--topic-tag", default=DEFAULT_TOPIC_TAG)
    parser.add_argument("--include-glob", default="*.md")
    parser.add_argument("--skip-names", default=DEFAULT_OUTPUT.name)
    ...
    note_paths = find_notes(
        args.content_dir,
        include_glob=args.include_glob,
        skip_names=parse_list_arg(args.skip_names),
    )
